...
const reducer = (state, action) => {
  switch(action.type){
    case 'SET': return {...state, data: action.todo}
    case 'DELETE': return {...state, data: state.data.filter((item) => item.id !== action.todo)}
...
const SecondScreen = ( {navigation, route} ) => {
    ...
    const fetchData = async () =>{
        await fetch('https://670cb2ec7e5a228ec1d10fcb.mockapi.io/toDoList/toDoList')
            .then(res=>res.json()).then((data) => dispatch({type: 'SET', todo: data}))
    }
